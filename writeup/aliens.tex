...
\abstract{Information about the world is not always stated explicitly. We investigated whether children can infer generalizable information pragmatically from speakers' word choices. In four experiments, we introduced children to novel objects that were described with prenominal adjectives (``a small tibu'') and asked whether they would infer that typical category exemplars contrasted on the named dimension (e.g. that tibus are usually larger). Preschoolers from a university nursery school and local children's museum reliably inferred implied contrast with the support of framing cues (Experiment 1).  Performance was reduced after removing the contrastive framing, but improved after a pre-exposure to the relevant adjective pairs (Experiment 2). Contrast inferences did not differ even when the polarity of the adjectives was reversed (Experiment 3), and in a free-response task, children spontaneously produced relevant property contrasts (Experiment 4). Our findings suggest that preschoolers can generalize from a single exemplar based on how it is described---and their generalizations encompass not just what the category is, but also what it isn't. More generally, sensitivity to the implications of speakers' production choices may help children learn about the world. 
...
% In Experiment 1, we provided a supportive scaffolding to help children recognize that adjectives were being used contrastively: We told them explicitly that ``This is a special kind of tibu.'' 
...
%, even without the cue of contrastive framing (i.e. without reference to the exemplar being ``special'').
